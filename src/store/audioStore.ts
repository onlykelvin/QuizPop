...
export const useAudioStore = create<AudioStore>((set, get) => {
  ...
  const scorePlusSound = new Audio('/assets/audio/sfx/scorePlus.mp3');
  const scoreMinusSound = new Audio('/assets/audio/sfx/scoreMinus.mp3');
  ...
  return {
    ...
    playScorePlus: () => {
      if (!get().isMuted) {
        scorePlusSound.volume = get().volume;
        scorePlusSound.currentTime = 0; // Reset to start to allow rapid playback
        scorePlusSound.play().catch(console.error);
      }
    },

    playScoreMinus: () => {
      if (!get().isMuted) {
        scoreMinusSound.volume = get().volume;
        scoreMinusSound.currentTime = 0; // Reset to start to allow rapid playback
        scoreMinusSound.play().catch(console.error);
      }
    },
